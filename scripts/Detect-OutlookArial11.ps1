# Outlook Classic Arial 11 - DETECTION
    ...
    foreach ($Name in $Expected.Keys) {
        ...
        try {
            ...
            if (-not (Test-BytesEqual -Actual $Actual -Expected $Wanted)) {
                $Issues.Add(
                    "SID=${Sid}; Value=${Name}; Reason=Value does not match Arial 11 baseline"
                )
            }
        }
        catch {
...
Write-Output 'Compliant: Outlook Classic new mail, replies/forwards and plain text are configured for Arial 11.'
exit 0
